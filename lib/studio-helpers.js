...
function enrichArtifactForUi(id, view, manifest, authoredArtifacts = null) {
  ...
  if (artifact.type === 'condition') {
    ...
    return {
      artifact,
      source: sourceArtifact(id, authoredArtifacts),
      compiled: model ? {
        ...model,
  ...
  return {
    artifact,
    source: sourceArtifact(id, authoredArtifacts),
    compiled: artifact.type === 'pipeline' ? { steps: view.getPipelineSteps(id) } : null,
    display,
  };
}

function sourceArtifact(id, artifacts) {
...
module.exports = { listEntrypoints, analyzePipeline, buildTree, buildFlowModel, enrichArtifactForUi, sourceArtifact, loadSamples, analysisSummary };
