...
const { listEntrypoints, analyzePipeline, buildTree, buildFlowModel, enrichArtifactForUi, sourceArtifact, loadSamples, analysisSummary } = require('./studio-helpers');
...
function startSandbox(project, options = {}) {
  ...
  app.get('/api/pipelines/:id', (req, res) => {
    ...
    res.json({
      ...
      display,
      stats,
      source: sourceArtifact(id, ctx.authoredArtifacts),
    });
  });
  ...
  app.get('/api/rules/:id', (req, res) => {
    ...
    res.json(enrichArtifactForUi(id, ctx.view, ctx.manifest, ctx.authoredArtifacts));
  });
...
function compileIntoCtx(project, ctx, verbose = false) {
  ...
  ctx.authoredArtifacts = bundle.artifacts;
