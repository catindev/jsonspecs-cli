...
function startSandbox(project, options = {}) {
  ...
  app.post('/api/playground/run', (req, res) => {
    const body = req.body ?? {};
    // Старые сохранённые Sandbox-запросы могли держать selector внутри context.
    // Нативный интерфейс rules v4 принимает pipelineId только на верхнем уровне tuple.
    const pipelineId = body.pipelineId || body.context?.pipelineId;
    if (!pipelineId || typeof pipelineId !== 'string') {
      return res.status(400).json({ message: 'pipelineId is required (string)' });
    }
    const payload = body.payload === undefined ? {} : body.payload;
    let context = body.context === undefined ? {} : body.context;
    if (body.pipelineId === undefined && context && typeof context === 'object' && !Array.isArray(context)) {
      context = { ...context };
      delete context.pipelineId;
    }
    try {
